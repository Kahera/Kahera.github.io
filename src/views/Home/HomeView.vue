...
const interests = ref([
   { id: 'sewing', tone: 'rose', tinted: true, symbol: 'content_cut' },
   { id: 'printing', tone: 'blue', tinted: false, symbol: 'view_in_ar' },
   { id: 'making', tone: 'green', tinted: true, symbol: 'handyman' },
   { id: 'beekeeping', tone: 'honey', tinted: false, symbol: 'hive' },
   { id: 'other', tone: 'plum', tinted: true, symbol: 'interests' }
] as const);
</script>
   ...
   <div>
      <section aria-labelledby="site-title"
         ...
         <div
            ...
            <div
               class="portrait-composition relative isolate order-first aspect-210/245 w-40 justify-self-center overflow-hidden rounded-b-full sm:order-0 sm:w-full sm:justify-self-end">
               ...
               <img :src="portrait" :alt="t('home.portraitAlt')" width="190" height="245" fetchpriority="high"
      ...
      <section id="work" aria-labelledby="work-title" class="focus-group bg-surface dark:bg-surface-dark">
         ...
         <FocusSection v-for="(section, index) in workSections" :id="section.id" :key="section.id"
            :eyebrow="t(`${section.contentKey}.eyebrow`)" :heading="t(`${section.contentKey}.heading`)" tone="blue"
            :symbol="section.symbol" :tinted="section.tinted" :reverse="index % 2 === 0">
            ...
            <template v-if="section.id === 'work-development'" #links>
               <a :href="t('links.linkedIn')" target="_blank" rel="noopener noreferrer" class="underline">
                  {{ t('home.work.link') }}
               </a>
            </template>
         </FocusSection>
      ...
      <section id="interests" aria-labelledby="interests-title"
         ...
         <FocusSection v-for="(interest, index) in interests" :id="interest.id" :key="interest.id"
            :eyebrow="t(`home.${interest.id}.eyebrow`)" :heading="t(`home.${interest.id}.heading`)"
            :description="t(`home.${interest.id}.body`)" :tone="interest.tone" :tinted="interest.tinted"
            :reverse="(workSections.length + index) % 2 === 0" :symbol="interest.symbol" />
      </section>
   </div>
</template>
