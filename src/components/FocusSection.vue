   ...
   <section :id="id" :aria-labelledby="`${id}-title`"
      ...
      <div class="page-width grid items-center gap-6 py-9 md:grid-cols-2 md:gap-12 md:py-12 lg:gap-16 lg:pl-24">
         <div :class="reverse ? 'md:col-start-2 md:row-start-1' : 'md:col-start-1 md:row-start-1'">
            <p class="mb-4 flex items-center gap-4 text-xs font-semibold uppercase before:h-0.5 before:w-8 before:shrink-0 before:bg-current"
               :class="tones[tone]">
               {{ eyebrow }}
            </p>
            ...
            <div v-if="$slots.links" class="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-base" :class="tones[tone]">
               <slot name="links" />
            </div>
         </div>
         <div :class="reverse ? 'md:col-start-1 md:row-start-1' : 'md:col-start-2 md:row-start-1'">
            <slot name="media">
               <figure v-if="image">
                  ...
                  <figcaption v-if="image.caption" class="border-t-4 pt-3 font-display text-sm"
